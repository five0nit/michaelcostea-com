(() => {
  ...
  if (!hasResult) {
    status.textContent = "No Threads authorization response was supplied. Start connection from the local Social Engine.";
    return;
  }

  const localUrl = `http://127.0.0.1:8770/callback?${forwarded.toString()}`;
  const goLocal = () => window.location.replace(localUrl);

  status.textContent = forwarded.has("error")
    ? "Threads returned an authorization error. Returning the result to the local Social Engine…"

...
const expectedOwnedGuidePath = 'guides/ai-agent-execution-receipt-template/?utm_source=michaelcostea.com&utm_medium=owned_resources&utm_campaign=safe_walk_away_launch';
const ownedGuideCard = ownedGuidePath?.closest('.welcome-resource-cta');
if (!ownedGuidePath || ownedGuidePath.getAttribute('href') !== expectedOwnedGuidePath || ownedGuidePath.textContent.trim() !== 'Open validator') {
  throw new Error('qualified Resources-to-guide path missing or incorrect');
}
if (!ownedGuideCard || ownedGuideCard.getAttribute('aria-label') !== 'AI agent execution receipt validator' || ownedGuideCard.querySelector('.resource-badge')?.textContent.trim() !== 'FREE BROWSER TOOL' || ownedGuideCard.querySelector('b')?.textContent.trim() !== 'AI Agent Execution Receipt Validator' || !ownedGuideCard.closest('#resourcesWindow')) {
  throw new Error('secondary Resources validator framing missing or incorrect');
}

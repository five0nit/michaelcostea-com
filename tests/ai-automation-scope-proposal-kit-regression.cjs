...
const expectedTitle='AI Automation Proposal Template + Scope of Work Generator';
if(document.title!==expectedTitle) throw new Error(`unexpected title ${document.title}`);
const metaDescription=attr('meta[name="description"]','content');
for(const term of ['AI automation proposal generator','freelancers and agencies','scope of work','acceptance plan']) if(!metaDescription.includes(term)) throw new Error(`meta description missing buyer-intent term: ${term}`);
...
for(const phrase of ['AI Automation Scope-to-Proposal Kit','Turn one client call into an AI automation scope of work and proposal.','Stop starting from a blank proposal template.','US$12','US$3','Code SCOPE3','US$9 fixed discount','maximum 10 redemptions','before applicable taxes','Offline browser app','six aligned client drafts','14-day description-match refund policy','No guaranteed ROI or outcome','Live · buyer checkout verified · ZIP delivery attached']) if(!body.includes(phrase)) throw new Error(`missing phrase: ${phrase}`);
...
if(!sitemap.includes('<loc>https://michaelcostea.com/ai-automation-scope-proposal-kit.html</loc>')) throw new Error('canonical landing missing from sitemap');

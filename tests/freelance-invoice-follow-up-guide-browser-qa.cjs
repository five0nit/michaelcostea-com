...
const url=process.env.INVOICE_GUIDE_URL||'http://127.0.0.1:4192/guides/freelance-invoice-follow-up-email/';
const out=process.env.INVOICE_GUIDE_QA_OUT||'/tmp/freelance-invoice-follow-up-guide-qa';
...
(async()=>{
  ...
  for(const [name,viewport] of Object.entries({desktop:{width:1440,height:1000},mobile:{width:390,height:844}})){
    ...
    const metrics=await page.evaluate(()=>({
      ...
      measuredLinks:[...document.querySelectorAll('a[data-analytics-event]')].map(link=>({eventName:link.dataset.analyticsEvent,itemId:link.dataset.analyticsItemId})),
    ...
    if(!metrics.h1||metrics.templates!==3||!metrics.analyticsLoader) throw new Error(`${name}: content or analytics loader missing`);
    ...
    if(metrics.measuredLinks.length!==6||metrics.measuredLinks.some(link=>!link.itemId)) throw new Error(`${name}: measured-link contract failed`);
    ...
    await page.screenshot({path:path.join(out,`freelance-invoice-follow-up-guide-${name}.png`),fullPage:true});
